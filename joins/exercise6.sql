...
SELECT mems.firstname ||' '|| mems.surname AS member, facs.name AS facilities,
CASE 
WHEN mems.id = 0 
THEN facs.guest_cost * books.slots
ELSE 
facs.member_cost * books.slots
END AS cost FROM cd.members mems
JOIN cd.bookings books ON books.id = mems.id 
JOIN cd.facilities facs ON facs.facility_id = books.facility_id
WHERE books.starttime >= '2012-09-14' AND books.starttime < '2012-09-15' 
AND ((mems.id = 0 AND facs.guest_cost * books.slots > 30) 
	 OR (mems.id != 0 AND facs.member_cost * books.slots > 30))
ORDER BY cost DESC	 