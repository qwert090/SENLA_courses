...
SELECT dategen.date, (SELECT SUM(
  CASE WHEN id = 0 THEN slots*guest_cost
  ELSE slots*member_cost END
		  ) AS sumrev FROM cd.bookings books
JOIN cd.facilities facs ON facs.facility_id = books.facility_id	
WHERE books.start_time > dategen.date - INTERVAL '14 days'
AND books.start_time < dategen.date + INTERVAL '1 day')/15 AS revfor15
FROM
