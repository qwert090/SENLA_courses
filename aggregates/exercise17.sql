...
SELECT facility_id, totalslots 
FROM(
  SELECT facility_id, totalslots, RANK() OVER(ORDER BY totalslots DESC) AS runkedslots  
	FROM (
  		   SELECT facility_id, SUM(slots) AS totalslots FROM cd.bookings
