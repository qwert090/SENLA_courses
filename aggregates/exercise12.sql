...
WITH sum AS (
  SELECT facility_id, EXTRACT(month FROM start_time) AS month, slots FROM cd.bookings
  WHERE start_time >= '01-01-2012' AND start_time < '01-01-2013'
  )
SELECT facility_id, month, SUM(slots)  FROM sum sum1
GROUP BY facility_id, month
UNION ALL 
SELECT facility_id, null, SUM(slots) FROM sum sum2
GROUP BY facility_id
UNION ALL 
SELECT null, null, SUM(slots) FROM sum sum2
