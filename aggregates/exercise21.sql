...
SELECT name, initialoutlay/(revenue - monthly_maintenance) AS month FROM
(SELECT name, initialoutlay, monthly_maintenance, (SUM(sumcost))/3 AS revenue FROM
(SELECT name, initialoutlay, monthly_maintenance, 
CASE WHEN id = 0 THEN slots*guest_cost
ELSE slots*member_cost END AS sumcost
FROM cd.facilities facs
JOIN cd.bookings books ON books.facility_id = facs.facility_id) AS fasilities
GROUP BY name, initialoutlay, monthly_maintenance) AS sumcost
ORDER BY name