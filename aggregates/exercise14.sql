...
WITH atsep AS(
SELECT mems.surname, mems.firstname, mems.id, books.start_time FROM cd.members mems
JOIN cd.bookings books ON books.id = mems.id
WHERE books.start_time >= '2012-09-01'  
)
SELECT surname, firstname, id, MIN(start_time) FROM atsep
GROUP BY surname, firstname, id
ORDER BY id