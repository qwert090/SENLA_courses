...
AS (
  SELECT recommended_by FROM cd.members 
  WHERE id = 27
  UNION ALL
  SELECT mems.recommended_by 
  FROM recommenders recs
  JOIN cd.members mems
  ON mems.id = recs.recommender
	)  
SELECT recs.recommender, mems.firstname, mems.surname FROM recommenders recs
JOIN cd.members mems ON recs.recommender = mems.id