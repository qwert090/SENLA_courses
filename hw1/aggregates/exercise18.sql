...
FROM(SELECT firstname, surname, ((SUM(books.slots)+10)/20)*10 AS hours FROM cd.bookings books
JOIN cd.members mems ON mems.id = books.id
GROUP BY firstname, surname) AS athours
