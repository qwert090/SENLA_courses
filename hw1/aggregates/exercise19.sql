...
(SELECT name, rank() over(ORDER BY sumcost DESC) FROM 
(SELECT name, SUM(cost) AS sumcost FROM 
(SELECT name, 
CASE WHEN id = 0 THEN slots*guest_cost
ELSE slots*member_cost END AS cost FROM cd.facilities facs
JOIN cd.bookings books ON books.facility_id = facs.facility_id) AS sum
GROUP BY name) AS totalsum
)AS ranked
WHERE rank <=3