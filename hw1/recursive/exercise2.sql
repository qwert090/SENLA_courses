...
WITH RECURSIVE recommendeds(id) 
AS (
  SELECT id FROM cd.members 
  WHERE recommended_by = 1
  UNION ALL
  SELECT mems.id
  FROM recommendeds recs
  JOIN cd.members mems
  ON mems.recommended_by = recs.id
	)  
SELECT recs.id, mems.firstname, mems.surname FROM recommendeds recs
JOIN cd.members mems ON recs.id = mems.id
ORDER BY id